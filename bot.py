...
LOG_CHANNEL_ID = int(os.getenv('TELEGRAM_LOG_CHANNEL_ID', '0'))
...
async def handle_message(update: Update, context):
    if update.message.photo:
        ...
        try:
            ...
            await update.message.reply_text("❌ There was an issue forwarding your screenshot to the admin.")
        finally:
        ...
        if match:
            ...
            try:
                ...
                await update.message.reply_text("✅ Your payment request has been received. Please reply with a screenshot if available.")
            except Exception as e:
                logger.error(f"Error processing request: {e}", exc_info=True)
                await update.message.reply_text("❌ An error occurred while processing your request.")
        else:
            await update.message.reply_text("❌ Invalid format. Please type /start to see the correct format.")
...
async def button_callback(update: Update, context):
    ...
    try:
        ...
        if payment_request.get('status') != 'pending':
            await query.edit_message_text(f"Already {payment_request.get('status')} by {payment_request.get('admin_notes', 'another admin')}.")
            return
        ...
        await query.edit_message_text("❌ Error processing the request.")
